...
if (!class_exists('AddThisRegistrationFeature')) {
    ...
    class AddThisRegistrationFeature extends AddThisFeature
    {
        ...
        protected function registerAjaxEndpoints()
        {
            ...
            add_action('wp_ajax_addthis_validate_profile_id', array($this, 'printValidateProfileIdProxy'));
            ...
            add_action('wp_ajax_addthis_boost_update', array($this, 'updateBoostProxy'));
            add_action('wp_ajax_addthis_boost_compatibility', array($this, 'checkBoostCompatibilityProxy'));
            add_action('wp_ajax_addthis_get_follow_services', array($this, 'getFollowServicesProxy'));
            add_action('wp_ajax_addthis_get_share_services', array($this, 'getShareServicesProxy'));
            add_action('wp_ajax_addthis_get_promoted_url', array($this, 'getPromotedUrl'));
            add_action('wp_ajax_addthis_add_promoted_url', array($this, 'addPromotedUrl'));
            add_action('wp_ajax_addthis_delete_promoted_url', array($this, 'deletePromotedUrl'));
        }
         ...
         * profile id. Only used for profile id and profile type validation. For
         * settings, use printGetBoostConfigProxy
         *
         * @return null
         */
        public function printValidateProfileIdProxy()
        {
            $required = array('profileId', 'plugin_version', 'plugin_pco');
            $input = $this->jsonSetup($required);
            $this->checkForEditPermissions();

            $profileId = urlencode($input['profileId']);
         ...
         * This must be public as it's used in a callback for add_action
         *
         * Updates settings for a tool
         *
         * @return null
         */
        public function updateBoostProxy()
        {
            $required = array('nonce', 'pco', 'settings', 'plugin_version', 'plugin_pco');
            $input = $this->jsonSetup($required);
            $this->checkForEditPermissions();
            ...
            if (isset($configs['api_key'])
                && isset($configs['addthis_profile'])
            ) {
                $url = $this->globalOptionsObject->getDarkseidBaseUrl()
                    . 'plugins/' . $plugin_pco
                    . '/v/' . $version
                    . '/site/' . $configs['addthis_profile']
                    . '/widget';
         ...
         * This must be public as it's used in a callback for add_action
         *
         * Get a list of follow services straight from AddThis, then we don't have to update the plugin for new ones
         *
         * @return null
         */
        public function getFollowServicesProxy()
        {
            $required = array('nonce');
            $input = $this->jsonSetup($required);

            $result = array();
            $urlRoot = 'https://cache.addthiscdn.com';

            $configs = $this->globalOptionsObject->getConfigs();
            if (!empty($configs['debug_enable'])
                && !empty($configs['addthis_environment'])
            ) {
                $env = $configs['addthis_environment'];
                $urlRoot = 'http://cache-' . $env . '.addthiscdn.com';
            }

            $url = $urlRoot . '/services/v1/follow.en.json';
         ...
         * @return null
         */
        public function getShareServicesProxy()
        {
            $required = array('nonce');
            $input = $this->jsonSetup($required);
            ...
            $url = $urlRoot . '/services/v1/sharing.en.json';
         ...
         * This must be public as it's used in a callback for add_action
         *
         * Gets all promoted URL campaigns for recommended content tools
         *
         * @return null
         */
        public function getPromotedUrl()
        {
            $required = array('nonce');
            $input = $this->jsonSetup($required);
            $this->checkForEditPermissions();
            ...
            $url = $this->globalOptionsObject->getDarkseidBaseUrl()
              . 'wordpress/site/' . $profileId . '/campaigns';
         ...
         * This must be public as it's used in a callback for add_action
         *
         * Creates a promoted URL campaign on a recommended content tool
         *
         * @return null
         */
        public function addPromotedUrl()
        {
            $required = array('toolPco', 'url', 'nonce');
            $input = $this->jsonSetup($required);
            $this->checkForEditPermissions();
            $result = array();
            ...
            $url = $this->globalOptionsObject->getDarkseidBaseUrl()
              . 'wordpress/site/' . $profileId . '/campaigns/' . $input['toolPco'];

            $body = array($input['url']);
            ...
            if (is_wp_error($response)) {
                ...
                if (isset($result['data'][$input['toolPco']])) {
                    $result['success'] = true;
                }
         ...
         * This must be public as it's used in a callback for add_action
         *
         * Deletes a promoted URL campaign on a recommended content tool
         *
         * @return null
         */
        public function deletePromotedUrl()
        {
            $required = array('toolPco', 'nonce');
            $input = $this->jsonSetup($required);
            $this->checkForEditPermissions();
            $result = array();
